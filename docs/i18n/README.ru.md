...
| Android: артефакт MyID не резолвится при `repositoriesMode = FAIL_ON_PROJECT_REPOS` | Конфиг-плагин теперь автоматически добавляет репозиторий MyID в `dependencyResolutionManagement.repositories` в `settings.gradle`, если этот блок есть (и пропускает инъекцию в `allprojects`, чтобы не нарушить `FAIL_ON_PROJECT_REPOS`). Если вы изменили блок и резолвинг всё равно падает, добавьте `maven { url "https://artifactory.myid.uz/artifactory/myid" }` вручную. |
