...
| Android: `repositoriesMode = FAIL_ON_PROJECT_REPOS` ostida MyID artefakti resolve bo'lmaydi | Config plugin endi ushbu blok mavjud bo'lsa, MyID repozitoriysini `settings.gradle`dagi `dependencyResolutionManagement.repositories`ga avtomatik qo'shadi (va `FAIL_ON_PROJECT_REPOS`ni buzmaslik uchun `allprojects`ga qo'shishni o'tkazib yuboradi). Agar blokni o'zgartirgan bo'lsangiz va resolve baribir muvaffaqiyatsiz bo'lsa, `maven { url "https://artifactory.myid.uz/artifactory/myid" }` ni qo'lda qo'shing. |
