...
<p align="center">
  <img src="https://raw.githubusercontent.com/softwhere-uz/react-native-myid/main/docs/media/myid-demo.gif" alt="react-native-myid demo — one typed identify() call drives native MyID face liveness, then a typed result and typed MyIdError handling" width="720" />
</p>

<p align="center"><sub><em>Illustrative developer flow — the MyID SDK renders its own native liveness UI on device.</em></sub></p>
...
| Android: MyID artifact fails to resolve under `repositoriesMode = FAIL_ON_PROJECT_REPOS` | The config plugin now adds the MyID repo to `dependencyResolutionManagement.repositories` in `settings.gradle` automatically when that block exists (and skips the `allprojects` injection so it can't trip `FAIL_ON_PROJECT_REPOS`). If you customized the block and it still fails, add `maven { url "https://artifactory.myid.uz/artifactory/myid" }` there yourself. |
